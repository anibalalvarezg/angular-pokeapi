...
@Component({
  ...
  standalone: true,
  imports: [MatTableModule, MatPaginatorModule, MatButtonModule, ExtractPokemonIdPipe, MatInputModule, MatFormFieldModule, ReactiveFormsModule ],
...
export class PokemonTableComponent implements AfterViewInit {
  ...
  ngOnInit(): void {
    this.dataSource = new MatTableDataSource(this._store.selectSnapshot(PokemonState.getAllPokemons));
    this.searhControl.valueChanges.pipe(debounceTime(100)).subscribe((value: string) => {
